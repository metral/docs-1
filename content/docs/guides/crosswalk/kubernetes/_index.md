...
Create, deploy, and manage production-ready infrastructure leveraging hosted
...
maintainability of the cluster, it's workloads, and the underlying
...
greatly improved to make the API more accessible and approachable to operators
and developers.

By reducing the Kubernetes API syntax used, including sane
defaults across resources, and maintaining idiomatic Kubernetes to make it
easier to work with the API, Crosswalk revamps the Kubernetes workflow while
still providing the full underlying API resource if necessary.

[Get started][pulumi-kx] with the `pulumi/kubernetesx` to manage Kubernetes
workloads using constructs built for everyone.

## Pulumi Query

Maintaining and understanding Kubernetes clusters requires coordination
and delivery of continuous changes. The API surface area for
any user becomes complex and highly disjointed when you want to make
sense of what is taking place in the cluster, and why it is happening.
...
across resources and reactive queries to understand what's happening under the hood.
...
[Get started][pulumi-kq] with the `pulumi/query` to understand Kubernetes
clusters and workloads through a new and proactive administrator lens.

## Join the Community

From provisioning managed Kubernetes clusters in a repeatable, and
production-ready way, to orchestrating workload deployments into Kubernetes in
a simplified and accessible approach, Pulumi's libraries ease
the management of infrastructure at scale, Kubernetes clusters and its workloads.
...
and playbooks to help you deliver production-ready Kubernetes.
